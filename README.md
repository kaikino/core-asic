...
timestamped trace buffer.  UART, SPI, I2C, JTAG, SWD, PS/2, CAN, low-speed
USB and 10 Mbit/s Manchester Ethernet frames (with hardware CRC) are
...
tools/      proto_asm.py (assembler/disassembler), proto_ref.py (cycle-accurate model), tt_board_host.py (MicroPython driver for the demo board), mutate.py (mutation check)
examples/   microprograms: uart, spi (master + flash emulator), i2c (master + EEPROM emulator), jtag, swd, ps2, can, usb_ls, manchester (Ethernet), link_pulse, edge_timer, glitch_pulse
...
docs/       submission.md (the case), system-design.md (guided tour), info.md (datasheet), isa.md (ISA + host protocol), signoff.md (results)
```
...
New to the project? Start with `docs/submission.md` (the case for the design)
and `docs/system-design.md`, a piece-by-piece walkthrough for readers who
know basic SystemVerilog but have never designed a chip. `docs/isa.md` has
the instruction set and host protocol reference.
