...
# --- tdc_probe additions ---------------------------------------------------
# The delay chain is fed by an asynchronous input; it must never be "repaired".
# A 200 ns max-delay keeps the path reported (so the chain delay is visible in
# the timing reports) without ever violating.
set_max_delay 200 -from [get_ports edge_in]
set_false_path -to [get_ports edge_out]
