...
# --- Programmable protocol emulator additions --------------------------------
# The delay lines (src/proto_delay_chain.sv, every net named *tdly_tap*) are
# asynchronous by design: the TDC chains are fed by raw input pads and the DTC
# chains deliberately delay an output by up to a clock period.  Neither path
# may be timed, repaired or buffered, so everything through a chain is a
# false path.  The normal (undelayed) paths to the same output pins remain
# constrained, and RSZ_DONT_TOUCH_RX in src/config.json protects the cells.
set_false_path -through [get_nets *tdly_tap*]
